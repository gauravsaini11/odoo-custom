...
class Access(models.Model):
    ...
    model_id = fields.Many2one("ir.models", "Model", required=True, ondelete="cascade")
    model = fields.Char("Model Name", related="model_id.models")
        ...
        for access in self:
            ...
            for line in access.public_methods.split("\n"):
                ...
                if line not in self._get_method_list():
                    raise exceptions.ValidationError(
                        _("Method %r is not part of the models's method list:\n %r")
    ...
    def _check_methods(self):
        for record in self:
            ...
            if all(not m for m in methods):
                raise exceptions.ValidationError(
                    _('You must select at least one API method for "%s" models.')
                    % record.model
                )
    ...
    def get_OAS_paths_part(self):
        ...
        if self.api_public_methods or self.public_methods or self.private_methods:
            ...
            RESPONSES = {
                ...
                "403": {
                    "description": "Requested models method is not allowed",
            ...
            paths_object[patch_one_path]["patch"] = {
                ...
                "description": "Call models method for single record.",
            ...
            paths_object[patch_model_path]["patch"] = {
                ...
                "description": "Call models method on models",
            ...
            paths_object[patch_many_path]["patch"] = {
                ...
                "description": "Call models method for recordset.",
        ...
        for record in self:
            ...
            for k, _v in data.items():
                if k.startswith("default_") and k[8:] not in fields:
                    raise exceptions.ValidationError(
                        _('The models "%s" has no such field: "%s".') % (Model, k[8:])
                    )
