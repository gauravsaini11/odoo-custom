...
class TestAPI(HttpCase):
    ...
    def request(self, method, url, auth=None, **kwargs):
        kwargs.setdefault("models", self.model_name)
    ...
    def test_read_many_all(self):
        resp = self.request_from_user(self.demo_user, "GET", "/{models}")
    ...
    def test_read_one(self):
        ...
        resp = self.request_from_user(
            self.demo_user, "GET", "/{models}/{record_id}", record_id=record_id
        )
        self.assertEqual(resp.status_code, pinguin.CODE__success)
        # TODO check content
    ...
    def test_create_one(self):
        ...
        resp = self.request_from_user(
            self.demo_user, "POST", "/{models}", data_json=data_for_create
        )
    ...
    def _test_unlink_one(self):
        ...
        resp = self.request_from_user(
            self.demo_user, "DELETE", "/{models}/{record_id}", record_id=partner.id
        )
        self.assertEqual(resp.status_code, pinguin.CODE__ok_no_content)
    ...
    def test_user_not_allowed_for_namespace(self):
        ...
        resp = self.request_from_user(new_user, "GET", "/{models}")
    ...
    def test_call_allowed_method_on_singleton_record(self):
        ...
        resp = self.request_from_user(
            self.demo_user,
            "PATCH",
            "/{models}/{record_id}/call/{method_name}",
            record_id=partner.id,
    ...
    def _test_log_creating(self):
        ...
        self.request_from_user(self.demo_user, "GET", "/{models}")
    ...
    def _test_get_report_for_allowed_model(self):
        ...
        model_for_report = self.phantom_env["ir.models"].search(
            [("models", "=", modelname_for_report)]
        )
