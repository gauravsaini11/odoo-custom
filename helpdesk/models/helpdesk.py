...
class HelpDeskTeam(models.Model):
    ...
    def _default_domain_member_id(self):
    ...
    assign_method = fields.Selection([
        ...
        compute='_compute_assign_methods', store=True, readonly=False, required=True,
    ...
    member_ids = fields.Many2many('res.users', string='Team Members', domain=lambda self: self._default_domain_member_id())
    visibility_member_ids = fields.Many2many('res.users', 'helpdesk_visibility_team', string='Team Visibility', domain=lambda self: self._default_domain_member_id(),
    ...
    has_external_mail_server = fields.Boolean(compute='_compute_external_mail_server')
    ...
    use_helpdesk_sale_timesheet = fields.Boolean(
        'Time Reinvoicing', compute='_compute_helpdesk_sale_timesheet', store=True,
    ...
    portal_show_rating = fields.Boolean(
        'Display Rating on Customer Portal', compute='_compute_portal_show', store=True,
        readonly=False)
    portal_rating_url = fields.Char('URL to Submit an Issue', readonly=True, compute='_compute_portal_rate')
    use_sla = fields.Boolean('SLA Policies')
    upcoming_sla_fail_tickets = fields.Integer(string='Upcoming SLA Fail Tickets', compute='_compute_sla_fail_tickets')
    unassigned_tickets = fields.Integer(string='Unassigned Tickets', compute='_compute_unassigned_ticket')
    ...
    def _compute_sla_fail_tickets(self):
    ...
    def _compute_unassigned_ticket(self):
    ...
    def _compute_assign_methods(self):
    ...
    def _compute_helpdesk_sale_timesheet(self):
    ...
    def _alias_get_creation_values(self):
        ...
        values['alias_model_id'] = self.env['ir.models']._get('helpdesk.ticket').id
        if self.id:
    ...
    def retrieve_dashboard(self):
        ...
        if self.env['helpdesk.team'].search([('use_rating', '=', True), '|', ('member_ids', 'in', self._uid), ('member_ids', '=', False)]):
            ...
            total_rating = self._compute_activities_avg(activity)
            total_activity_values = sum(activity.values())
            # In the 2 formula's below, we need to multiply at the end by (100 / MAX_SCORING)
            # where MAX_SCORING is defined in _compute_activities_avg as the value for a "great" rating.
    ...
    @api.model
    def _compute_activities_avg(self, activity):
...
class HelpDeskSeam(models.Model):
    ...
    def unlink(self):
        ...
        return super(HelpDeskSeam, stages).unlink()
