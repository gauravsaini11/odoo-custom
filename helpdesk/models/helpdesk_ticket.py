...
class HelpDeskSLAStatus(models.Model):
    ...
    deadline = fields.Datetime("Deadline", compute='_compute_deadlines', compute_sudo=True, store=True)
    reached_datetime = fields.Datetime("Reached Date", help="Datetime at which the SLA stage was reached for the first time")
    status = fields.Selection([('failed', 'Failed'), ('reached', 'Reached'), ('ongoing', 'Ongoing')], string="Status", compute='_compute_statu', compute_sudo=True, search='_search_status')
    color = fields.Integer("Color Index", compute='_compute_colours')
    exceeded_days = fields.Float("Excedeed Working Days", compute='_compute_exceededdays', compute_sudo=True, store=True, help="Working days exceeded for reached SLAs compared with deadline. Positive number means the SLA was eached after the deadline.")
    ...
    @api.depends('deadline', 'reached_datetime')
    def _compute_exceededdays(self):
        for status in self:
    ...
    def _get_freezed_hours(self, working_calendar):
        ...
        field_stage = self.env['ir.models.fields']._get(self.ticket_id._name, "stage_id")
...
class HelpDeskTicket(models.Model):
    ...
    kanban_state_label = fields.Char(compute='_compute_kanban_state', string='Column Status', tracking=True)
    legend_blocked = fields.Char(related='stage_id.legend_blocked', string='Kanban Blocked Explanation', readonly=True, related_sudo=False)
    legend_done = fields.Char(related='stage_id.legend_done', string='Kanban Valid Explanation', readonly=True, related_sudo=False)
    legend_normal = fields.Char(related='stage_id.legend_normal', string='Kanban Ongoing Explanation', readonly=True, related_sudo=False)
    domain_user_ids = fields.Many2many('res.users', compute='_compute_user_domain_ids')
    user_id = fields.Many2one(
        'res.users', string='Assigned to', compute='_compute_user_stage_ids', store=True,
        readonly=False, tracking=True,
        domain=lambda self: [('groups_id', 'in', self.env.ref('helpdesk.group_helpdesk_user').id)])
    partner_id = fields.Many2one('res.partner', string='Customer')
    partner_ticket_count = fields.Integer('Number of closed tickets from the same partner', compute='_compute_partner_ticket')
    attachment_number = fields.Integer(compute='_compute_attachment', string="Number of Attachments")
    is_self_assigned = fields.Boolean("Am I assigned", compute='_compute_self_assigned')
    ...
    stage_id = fields.Many2one(
        'helpdesk.stage', string='Stage', compute='_compute_user_stage_ids', store=True,
    ...
    assign_hours = fields.Integer("Time to first assignment (hours)", compute='_compute_assignhours', store=True, help="This duration is based on the working calendar of the team")
    close_date = fields.Datetime("Close date", copy=False)
    close_hours = fields.Integer("Time to close (hours)", compute='_compute_closehours', store=True, help="This duration is based on the working calendar of the team")
    open_hours = fields.Integer("Open Time (hours)", compute='_compute_openhours', search='__search_openhours', help="This duration is not based on the working calendar of the team")
    # SLA relative
    ...
    sla_reached_late = fields.Boolean("Has SLA reached late", compute='_compute_sla_reached', compute_sudo=True, store=True)
    ...
    sla_fail = fields.Boolean("Failed SLA Policy", compute='_compute_slafails', search='_search_slafails')
    ...
    website_message_ids = fields.One2many(domain=lambda self: [('models', '=', self._name), ('message_type', 'in', ['email', 'comment'])])
    ...
    @api.model
    def _search_slafails(self, operator, value):
        datetime_now = fields.Datetime.now()
    ...
    @api.depends('user_id')
    def _compute_self_assigned(self):
        for ticket in self:
            ticket.is_self_assigned = self.env.user == ticket.user_id

    @api.depends('team_id')
    def _compute_user_stage_ids(self):
    ...
    @api.depends('partner_id')
    def _compute_partner_ticket(self):
    ...
    @api.model
    def __search_openhours(self, operator, value):
    ...
    def _track_template(self, changes):
        ...
        if 'stage_id' in changes and ticket.stage_id.template_id:
            res['stage_id'] = (ticket.stage_id.template_id, {
                ...
                'subtype_id': self.env['ir.models.data'].xmlid_to_res_id('mail.mt_note'),
