...
class AccountAnalytic(models.Model):
    ...
    def default_get(self, field_list):
        result = super(AccountAnalytic, self).default_get(field_list)
    ...
    def _domain_employees_id(self):
        if not self.user_has_groups('hr_timesheet.group_hr_timesheet_approver'):
            return [('user_id', '=', self.env.user.id)]
        return []

    def _domain_tasks_id(self):
        if not self.user_has_groups('hr_timesheet.group_hr_timesheet_approver'):
            return ['|', ('privacy_visibility', '!=', 'followers'), ('allowed_user_ids', 'in', self.env.user.ids)]
        return []

    task_id = fields.Many2one(
        'project.task', 'Task', compute='_compute_task', store=True, readonly=False, index=True,
        domain="[('company_id', '=', company_id), ('project_id.allow_timesheets', '=', True), ('project_id', '=?', project_id)]")
    project_id = fields.Many2one(
        'project.project', 'Project', compute='_compute_project', store=True, readonly=False,
        domain=_domain_projects_id)
    user_id = fields.Many2one(compute='_compute_users_id', store=True, readonly=False)
    employee_id = fields.Many2one('hr.employee', "Employee", domain=_domain_employees_id, context={'active_test': False})
    department_id = fields.Many2one('hr.department', "Department", compute='_compute_departments_id', store=True, compute_sudo=True)
    encoding_uom_id = fields.Many2one('uom.uom', compute='_compute_encoding_uom')
    partner_id = fields.Many2one(compute='_compute_partner', store=True, readonly=False)

    def _compute_encoding_uom(self):
    ...
    def create(self, vals_list):
        ...
        # this make a second loop on the vals, we need to wait preprocess as it could change company_id in the vals
        ...
        lines = super(AccountAnalytic, self).create(vals_list)
    ...
    @api.model
    def fields_views_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        """ Set the correct label for `unit_amount`, depending on company UoM """
        result = super(AccountAnalytic, self).fields_views_get(view_id=view_id, view_type=view_type, toolbar=toolbar, submenu=submenu)
        result['arch'] = self._apply_timesheet_labels(result['arch'], view_type=view_type)
        return result

    @api.model
    def _apply_timesheet_labels(self, view_arch, view_type='form'):
    ...
    def _is_timesheet_encode_uom_days(self):
