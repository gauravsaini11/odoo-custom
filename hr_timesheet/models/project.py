...
class Project(models.Model):
    ...
    encode_uom_in_days = fields.Boolean(compute='_is_timesheet_encode_uom_dayss')

    def _is_timesheet_encode_uom_dayss(self):
...
class Task(models.Model):
    ...
    total_hours_spent = fields.Float("Total Hours", compute='_compute_total_hours_spents', store=True, help="Time spent on this task, including its sub-tasks.")
    ...
    encode_uom_in_days = fields.Boolean(compute='_is_timesheet_encode_uom_dayss', default=lambda self: self._uom_in_days())
    ...
    def _compute_total_hours_spents(self):
        for task in self:
    ...
    def action_view_subtasks_timesheet(self):
        self.ensure_one()
    ...
    @api.model
    def _fields_views_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        """ Set the correct label for `unit_amount`, depending on company UoM """
        result = super(Task, self)._fields_views_get(view_id=view_id, view_type=view_type, toolbar=toolbar, submenu=submenu)
        result['arch'] = self.env['account.analytic.line']._apply_timesheet_labels(result['arch'])
