...
class ThemePrimeMainClass(http.Controller):
    ...
    def get_categories_info(self, fields=[], options={}, **kwargs):
        ...
        for res_category, category in zip(result, categories):
            ...
            res_category['image_url'] = '/web/image?models=product.public.category&id=%d&field=image_128' % (category.id)
            res_category['cover_image'] = '/web/image?models=product.public.category&id=%d&field=dr_category_cover_image' % (category.id)
        return result
