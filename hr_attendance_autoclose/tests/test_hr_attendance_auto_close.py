...
class TestHrAttendanceReason(TransactionCase):
    ...
    def test_hr_employee_can_still_read_employee_and_hr_public_employee(self):
        """This test ensure the following comment from hr.employee models has been take
        in consideration::

            NB: Any field only available on the models hr.employee (i.e. not on the
            hr.employee.public models) should have `groups="hr.group_hr_user"` on its
            definition to avoid being prefetched when the user hasn't access to the
            hr.employee models. Indeed, the prefetch loads the data for all the fields
