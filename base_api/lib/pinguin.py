...
def validate_spec(model, spec):
    """Validates a spec for a given models.
    :param object model: (:obj:`Model`) The models against which to validate.
...
def transform_dictfields_to_list_of_tuples(record, dct, ENV=False):
    ...
    :param odoo.models.Model record: The models object.
...
# Get a models with special context
def get_model_for_read(model, ENV=False):
    """Fetch a models object from the environment optimized for read.
    ...
    :param str model: The models to retrieve from the environment.
    :param object env: Environment
    :returns: the framework models if exist, otherwise raises.
    :rtype: odoo.models.Model
    :raise: werkzeug.exceptions.HTTPException if the models not found in env.
    """
    if ENV:
        return ENV[model]
    ...
    try:
        ...
        err[2] = 'The "%s" models is not available on this instance.' % model
        ...
        # Normal field, or unspecified relational
            ...
            if not hasattr(record, field):
                raise odoo.exceptions.ValidationError(
                    odoo._('The models "%s" has no such field: "%s".')
                    % (record._name, field)
                )
