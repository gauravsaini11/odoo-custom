...
def pre_init_hook_for_submodules(cr, model, field):
    ...
    :param str field:
        Binary field that had the images in that :param:`models`, like
        ``image``.
    """
...
def uninstall_hook_for_submodules(
    ...
    :param str field_medium:
        Binary field that had the medium-sized images in that :param:`models`,
        like ``image_medium``.

    :param str field_small:
        Binary field that had the small-sized images in that :param:`models`,
        like ``image_small``.
    """
    env = api.Environment(cr, SUPERUSER_ID, {})
    with cr.savepoint():
