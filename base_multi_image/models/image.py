...
class Image(models.Model):
    ...
    _description = """ image models for multiple image functionality """
    _sql_constraints = [
        (
            "uniq_name_owner",
    ...
    def _selection_owner_ref_id(self):
        """Allow any models; after all, this field is readonly."""
        return [(r.model, r.name) for r in self.env["ir.models"].search([])]

    @api.depends("owner_model", "owner_id")
    def _compute_owner_ref_id(self):
        """Get a reference field based on the split models and id fields."""
        for s in self:
            if s.owner_model:
    ...
    def _compute_show_technical(self):
        ...
        self.show_technical = all(
            "default_owner_%s" % f not in self.env.context for f in ("id", "models")
        )
