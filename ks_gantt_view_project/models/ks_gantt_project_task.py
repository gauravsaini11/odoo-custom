...
class KsProjectTask(models.Model):
    ...
    ks_start_datetime = fields.Datetime("Est Start Date", required=True, default=ks_default_start_date)
    ks_end_datetime = fields.Datetime("Est End Date", required=True, default=ks_default_end_datetime)
    ks_actul_start = fields.Datetime("Actul Start Date")
    ks_actul_end = fields.Datetime("Actul End Date")
    ...
    @api.onchange('ks_start_datetime', 'ks_end_datetime', 'ks_actul_start', 'ks_actul_end')
    def date_onchange(self):
        current_date = datetime.now()
        g = datetime.today().strftime(DEFAULT_SERVER_DATETIME_FORMAT)
        f = datetime.strptime(g, DEFAULT_SERVER_DATETIME_FORMAT)
        for rec in self:
            #if rec.ks_start_datetime and f:
            #    if rec.ks_start_datetime < f:
            #        raise ValidationError(_("Est Start Date can not be lower than current date"))
            if rec.ks_start_datetime and rec.ks_end_datetime:
                if rec.ks_start_datetime > rec.ks_end_datetime:
                    raise ValidationError(_("Est End Date not be lower than Est Start Date"))
            if rec.ks_actul_start and rec.ks_actul_end:
                if rec.ks_actul_start > rec.ks_actul_end:
                    raise ValidationError(_("Actual End Date not lower than Actual Start Date"))
            if rec.ks_end_datetime and rec.ks_actul_end:
                if rec.ks_start_datetime > rec.ks_actul_end:
                    raise ValidationError(_("Actual End Date not lower than Est Start Date"))
                                    
    # @api.onchange('ks_start_datetime','ks_end_datetime','ks_actul_start')
    # def date_onchange(self):

    #     current_date = datetime.now()
    #     for rec in self:
    #         if rec.ks_start_datetime > rec.ks_end_datetime:
    #            raise ValidationError(_("Est End Date can not be lower than Est Start Date"))
    #         if rec.ks_start_datetime < current_date:
    #            raise ValidationError(_("Est Start Date can not be lower than Current Date"))
    #         if rec.ks_actul_start < ks_start_datetime:
    #            raise ValidationError(_("Actul start date can not be lower than Est Start Date "))
